...
if(isset($_POST['reset'])){
	$FormData = array(
		...
		'new_password' => htmlentities(mysqli_real_escape_string($connect, $_POST['password'])),
		'hashed_password' => hash('sha256', htmlentities(mysqli_real_escape_string($connect, $_POST['password']))),
	);
	...
	if(password_verify($Data['hosting_client_key'], $Key)){
		$sql = mysqli_query($connect,"UPDATE `hosting_clients` SET `hosting_client_password`='".$FormData['hashed_password']."' WHERE `hosting_client_key`='".$Data['hosting_client_key']."'");
		if($sql){
			$EmailTo = [['email' => $Email]];
			$Body = "
